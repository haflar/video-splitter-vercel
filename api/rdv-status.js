...
function requestOnce(url) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({
      ...
      headers: {
        'User-Agent': 'RDV-Passeport-Montreal/relay-1.0',
      ...
      let certificate;
      try {
        certificate = validatePeer(res.socket, u.hostname);
      } catch (error) {
        res.destroy();
        reject(error);
        return;
